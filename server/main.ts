...
process.umask(0o077);
async function main() {
  const dbPath = resolve(process.env.DATABASE_PATH ?? './data/chore.sqlite');
  ...
  if (process.argv.includes('--backup')) {
    const target = resolve(process.argv[process.argv.indexOf('--backup') + 1] ?? `${dirname(dbPath)}/backups/chore-${new Date().toISOString().replace(/[:.]/g, '-')}.sqlite`);
  ...
  console.log(`CHORE is listening at ${appUrl}. ${auth.needsSetup() ? 'Open the app to set your password.' : 'Authentication is enabled.'}`);
